...
const binariesPromiseByPreference = new Map<
  VideoHashFfmpegSourcePreference,
  Promise<PhashBinaries>
>();
...
export function getBundledFfmpegCandidatePaths(
  ...
  const resourcesPath =
    typeof options?.resourcesPath === "string"
...
function loadBundledBinaryPaths(): { ffmpegPath: string | null; ffprobePath: string | null } {
  ...
  const ffmpegPath =
    getBundledFfmpegCandidatePaths(vendorRelativePaths.ffmpegPath).find(isExecutablePath) ?? null;
  const ffprobePath =
    getBundledFfmpegCandidatePaths(vendorRelativePaths.ffprobePath).find(isExecutablePath) ?? null;
...
export function selectPhashBinaries(
  ...
  if (preference === "bundled") {
    ...
    throw new Error(
      "VideoHash ffmpeg source is forced to bundled, but bundled binaries are unavailable."
    );
  }
  ...
  if (preference === "system") {
    ...
    throw new Error(
      "VideoHash ffmpeg source is forced to system, but system ffmpeg/ffprobe are unavailable."
    );
  }
  ...
  if (
    ...
    bundledParsed &&
    systemParsed &&
    compareVersionTuple(systemParsed, bundledParsed) > 0
  ) {
    return system;
  }
...
async function resolveBinariesInternal(
  preference: VideoHashFfmpegSourcePreference
): Promise<PhashBinaries> {
...
export async function resolvePhashBinaries(
  preference?: VideoHashFfmpegSourcePreference
): Promise<PhashBinaries> {
