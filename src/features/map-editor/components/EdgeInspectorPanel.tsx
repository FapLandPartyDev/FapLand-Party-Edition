...
export const EdgeInspectorPanel: React.FC<EdgeInspectorPanelProps> = React.memo(
  ({ selectedEdge, allEdges, onPatchEdge, onDeleteEdge }) => {
    if (!selectedEdge) {
      return (
    ...
    const alternativeEdges = allEdges.filter(
      (edge) => edge.fromNodeId === selectedEdge.fromNodeId && edge.id !== selectedEdge.id
    );
      ...
      <div className="space-y-3 p-3">
        <div className="rounded-lg border border-white/6 bg-black/20 px-2.5 py-2">
          <p className="text-xs font-medium text-zinc-300">
            {selectedEdge.fromNodeId} <span className="text-zinc-600">→</span>{" "}
